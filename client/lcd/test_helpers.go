...
import (
	...
	fapp "github.com/forbole/forboled/app"
)
...
func InitializeTestLCD(t *testing.T, nValidators int, initAddrs []sdk.Address) (cleanup func(), validatorsPKs []crypto.PubKey, port string) {
	...
	privVal.Reset()
	db := dbm.NewMemDB()
	app := fapp.NewForboleApp(logger, db)
	cdc = fapp.MakeCodec() // XXX
	...
	for _, gdValidator := range genDoc.Validators {
		...
		appGenTx, _, _, err := fapp.ForboleAppGenTxNF(cdc, pk, pk.Address(), "test_val1", true)
		require.NoError(t, err)
		appGenTxs = append(appGenTxs, appGenTx)
	}

	genesisState, err := fapp.ForboleAppGenState(cdc, appGenTxs[:])
	require.NoError(t, err)
	...
	for _, addr := range initAddrs {
		...
		acc := fapp.NewGenesisAccount(&accAuth)
